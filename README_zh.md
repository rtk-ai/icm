...
一条命令配置 **17 个工具**（[完整集成指南](docs/integrations.md)）：

| 工具 | MCP | 钩子 | CLI | 技能 |
|------|:---:|:-----:|:---:|:------:|
| Claude Code | `~/.claude.json` | 5 个钩子 | `CLAUDE.md` | `/recall` `/remember` |
| Claude Desktop | JSON | — | — | — |
| Gemini CLI | `~/.gemini/settings.json` | 5 个钩子 | `GEMINI.md` | — |
| Codex CLI | `~/.codex/config.toml` | 4 个钩子 | `AGENTS.md` | — |
| Copilot CLI | `~/.copilot/mcp-config.json` | 4 个钩子 | `.github/copilot-instructions.md` | — |
| Cursor | `~/.cursor/mcp.json` | — | — | `.mdc` 规则 |
| Windsurf | JSON | — | `.windsurfrules` | — |
| VS Code | `~/Library/.../Code/User/mcp.json` | — | — | — |
| Amp | JSON | — | — | `/icm-recall` `/icm-remember` |
| Amazon Q | JSON | — | — | — |
| Cline | VS Code globalStorage | — | — | — |
| Roo Code | VS Code globalStorage | — | — | `.md` 规则 |
...
为所有支持的工具安装自动提取和自动召回钩子：

| 工具 | SessionStart | PreTool | PostTool | Compact | PromptRecall | 配置 |
|------|:-----------:|:-------:|:--------:|:-------:|:------------:|--------|
...
| 钩子 | 功能 |
|------|-------------|
| `icm hook start` | 在会话开始时注入关键/高重要性记忆的唤醒包（约 500 token） |
| `icm hook pre` | 自动允许 `icm` CLI 命令（无需权限提示） |
| `icm hook post` | 每 N 次调用从工具输出中提取事实（自动提取） |
| `icm hook compact` | 在上下文压缩前从对话记录中提取记忆 |
| `icm hook prompt` | 在每次用户提示开始时注入召回的上下文 |
...
| **适用工具** | Claude Code、Gemini、Codex、Copilot、OpenCode（通过钩子） | 全部 17 个兼容 MCP 的工具 |
...
所有 17 个工具共享同一个 SQLite 数据库。Claude 存储的记忆可立即被 Gemini、Codex、Copilot、Cursor 以及所有其他工具访问。
...
得分 = 60% 召回准确率 + 30% 事实细节 + 10% 速度。**98% 多智能体效率。**

## 为什么选择 ICM

| 能力 | ICM | Mem0 | Engram | AgentMemory |
|-----------|:---:|:----:|:------:|:-----------:|
| 工具支持 | **17** | 仅 SDK | ~6-8 | ~10 |
| 一键配置 | `icm init` | 手动 SDK | 手动 | 手动 |
| 钩子（启动时自动召回） | 5 个工具 | 无 | 通过 MCP | 1 个工具 |
| 混合搜索（FTS5 + 向量） | 30/70 加权 | 仅向量 | 仅 FTS5 | FTS5+向量 |
| 多语言嵌入 | 100+ 种语言（768d） | 视情况 | 无 | 英文 384d |
| 知识图谱 | Memoir 系统 | 无 | 无 | 无 |
| 时间衰减 + 整合 | 访问感知 | 无 | 基础 | 基础 |
| TUI 仪表板 | `icm dashboard` | 无 | 有 | Web 查看器 |
| 从工具输出自动提取 | 3 层，零 LLM | 无 | 无 | 无 |
| 反馈/纠正循环 | `icm_feedback_*` | 无 | 无 | 无 |
| 运行时 | Rust 单一二进制 | Python | Go | Node.js |
| 本地优先，零依赖 | SQLite 文件 | 云优先 | SQLite | SQLite |
...
| 文档 | 描述 |
|----------|-------------|
| [集成指南](docs/integrations.md) | 所有 17 个工具的配置：Claude Code、Copilot、Cursor、Windsurf、Zed、Amp 等 |
